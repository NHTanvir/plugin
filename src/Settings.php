...
class Settings extends Fields {
	...
	public function __construct( $args = [] ) {
		...
		$this->config = wp_parse_args( apply_filters( 'tan-settings-args', $args ), $defaults );
		$this->sections	= apply_filters( 'tan-settings-sections', $this->config['sections'] );
	...
	public function hooks() {
		...
		$this->priv( 'tan-settings', 'save_settings' );
		$this->priv( 'tan-reset', 'reset_settings' );
	}
	...
	public function save_settings() {
		...
		$is_savable		= apply_filters( 'tan-settings-savable', true, $option_name, $posted_data );

		if( ! $is_savable ) {
			wp_send_json( apply_filters( 'tan-settings-response', array( 'status' => -1, 'message' => __( 'Ignored' ) ), $this->sanitize( $_POST, 'array' ) ) );
		}
		...
		do_action( 'tan-settings-saved', $option_name, $posted_data );
		
		wp_send_json( apply_filters( 'tan-settings-response', array( 'status' => 1, 'message' => __( 'Settings Saved!' ), 'page_load' => $page_load ), $posted_data ) );
	}
	...
	public function reset_settings() {
		...
		$option_name	= $posted_data['option_name'];
		$is_savable		= apply_filters( 'tan-settings-resetable', true, $option_name, $posted_data );

		if( ! $is_savable ) {
			wp_send_json( apply_filters( 'tan-settings-response', array( 'status' => -1, 'message' => __( 'Ignored' ) ), $posted_data ) );
		}

		delete_option( $option_name );

		do_action( 'tan-settings-reset', $option_name );

		wp_send_json( apply_filters( 'tan-settings-response', array( 'status' => 1, 'message' => __( 'Settings Reset!' ) ), $posted_data ) );
	}
